...
BOOLEAN XGI_DySense( PHW_DEVICE_EXTENSION pHWDE , PUCHAR ujConnectStatus)
{
    ...
    OutputSelect = XGI330_OutputSelect ;
...
#ifdef WIN2000
        if ( temp == LCD_1600x1200 )
        {
            if (bGetDdcInfo ( HwDeviceExtension->pDevice, MASK_OUTPUTSTATE_CRT2LCD, pjEDIDBuf, ulBufferSize ) == TRUE)
            {
                pEDIDBuf = pjEDIDBuf + 0x48 ;
    	        for (i = 0; i < 3; i++)
                {
                   ucData = *( pEDIDBuf + 3 ) ;	
                   if ( ucData == 0xFD )
               	   {
               	      ucData = *( pEDIDBuf + 9 ) ;
               	      ulPixelClock = (ULONG)( ucData * 10 ) ;
               	      if ( ulPixelClock < 160 )
               	      {
               	          XGINew_SetRegOR( pVBInfo->P3d4 , 0x39 , ReduceTiming ) ;
               	          XGINew_SetRegOR( pVBInfo->P3d4 , 0x37 , EnableScalingLCD ) ;
               	      	  return( 1 ) ;
               	      }    	
                   }
                   pEDIDBuf = pEDIDBuf + 18 ;  
                }   
	    }
	}            
#endif		
